  ...
  <div className="container px-5 py-24 mx-auto">
    <div className="flex flex-wrap -m-4">
      {Object.keys(tshirts)?.map((item)=>{
        ...
        return  <div key={tshirts[item].id} className="lg:w-1/4 md:w-1/2 p-4 ">
        <Link href={`/product/${tshirts[item].id}`} className="block relative h-48 rounded overflow-hidden">
          <img alt="ecommerce" className="m-auto h-[50vh] md:h-[30vh] block" src={tshirts[item].img}/>
        </Link>
        <div className="mt-4">
          ...
          {tshirts[item].size.map((k)=>{
           return <span className=' border border-gray-600 px-1 mx-1'>{k}</span>
          })}
          <div className='mt-4'>
          {tshirts[item].color.map((k)=>{

           return <button style={{backgroundColor: `${k}`}} className=' border-2 rounded-full w-6 h-6 ' ></button>
          })}
          </div>
          </div>
        </div>
